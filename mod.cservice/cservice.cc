...
void cservice::checkChannelsFlood()
{
	...
	while (ptr != sqlChannelCache.end())
	{
		...
		{
			time_t iplasttime = theChan->getIPLastTime(itr->first);
			if (!iplasttime)
			{
				++itr;
				continue;
			...
			{
				theChan->RemoveFlooderIP(itr++->first);
			}
			else
				++itr;
		}
...
if (theUser->getFlag(sqlUser::F_AUTONICK))
{
	...
	if (tmpClient)
	{
		...
		{
			sqlUser* tmpUser2 = isAuthed(tmpClient, false);
			if (!tmpUser2)
			{
				generateNickName(tmpClient);
			}
		...
		Write("%s SN %s %s", getCharYY().c_str(), theClient->getCharYYXXX().c_str(), theUser->getNickName().c_str());
}
	/*
